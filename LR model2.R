...
# The problem statement is as follows - A digital media company  had launched a show. Initially, the show got a good response, but then witnessed a decline in viewership. 
...
ggplot(media, aes(day, Views_show)) + geom_line() +
...
# Weekday is negative, so it not weekdays but weekends are important
...
# Plot Model9 errors
ggplot(media, aes(day, error)) + geom_line() + 
...
ggplot(media, aes(day, Views_show)) + geom_line() + 
  scale_x_continuous(name = "days", breaks = seq(0,84,7), limits = c(0,84)) + 
  scale_y_continuous(name = "Views_show", breaks = seq(0,800000,100000), limits = c(0,800000)) + 
  geom_line(aes(x=day, y=Predicted_views_model5, colour="red"))
